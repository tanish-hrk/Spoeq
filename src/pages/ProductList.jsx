...
export default function ProductList(){
  ...
  try { const res = await api.get('/products/suggest?q='+encodeURIComponent(term)); setSuggestions(res.data); } catch(e){ void e; }
    }, 250);
  ...
  if(isLoading) return (
    <div className='p-6 max-w-6xl mx-auto'>
      <div className='h-9 w-40 mb-4 rounded bg-eco-fern/70 animate-pulse'></div>
      <div className='grid gap-4 grid-cols-2 md:grid-cols-3 lg:grid-cols-4'>
        {Array.from({ length:8 }).map((_,i)=>(
          <div key={i} className='rounded-xl p-4 border border-eco-grass/40 bg-eco-fern/60 dark:border-neutral-800 dark:bg-neutral-900 animate-pulse h-64'></div>
        ))}
      </div>
    </div>
  );
  ...
  return (
    <div className='p-6 max-w-6xl mx-auto'>
      {/* Compact top mini-rail */}
      <div className='mb-4 overflow-x-auto no-scrollbar'>
        <div className='flex gap-2 pr-2'>
          {['Fitness','Running','Training','Team Sports','Eco Gear','Sale'].map((c)=> (
            <button key={c} onClick={()=> setParam('q', c.split(' ')[0].toLowerCase())}
              className='text-[12px] px-3 py-1.5 rounded-full border border-eco-grass/50 bg-white/70 text-neutral-800 hover:border-eco-grass/80 dark:bg-neutral-900 dark:text-white dark:border-neutral-700'>
              {c}
            </button>
          ))}
        </div>
      ...
      <div className='grid gap-4 grid-cols-2 md:grid-cols-3 lg:grid-cols-4'>
        {data.items.map(p=> {
          ...
          return (
          <div key={p._id} className='group relative bg-white dark:bg-neutral-900 rounded-xl p-3 border border-neutral-200 dark:border-neutral-800 hover:shadow transition flex flex-col overflow-hidden'>
            <button
              onClick={async (e)=> {
